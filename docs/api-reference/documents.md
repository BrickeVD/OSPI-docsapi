...
Document metadata — datasheets, certificates, manuals, images — attached to products, with real file storage behind a swappable abstraction.
...
`documentType`, `lifecycleStatus`, `validFrom`/`validUntil`, `checksum` (SHA-256), `sortOrder`, and `certificateMetadata`.
